...
class Processor
{
    ...
    private function getVariableReferenceArgumentValue(VariableReference $variableReference, AbstractType $argumentType, Request $request)
    {
        ...
        if (TypeMap::KIND_LIST === $argumentType->getKind()) {
            if ((!$variable->isArray() && !\is_array($variable->getValue())) || ($variable->getTypeName() !== $argumentType->getNamedType()->getNullableType()->getName()) || (TypeMap::KIND_NON_NULL === $argumentType->getNamedType()->getKind() && $variable->isArrayElementNullable())
            ) {
